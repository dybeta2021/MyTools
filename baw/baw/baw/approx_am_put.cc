#include "interface.h"// define other option pricing formulas
...
double option_price_american_put_approximated_baw(const double &S,
    ...
    double g = 1;
    double gprime = 1;
    while ((fabs(g) > ACCURACY) && (fabs(gprime) > ACCURACY)// to avoid non-convergence
        ...
        gprime = (1.0 / q1 - 1.0) * (1.0 - exp((b - r) * time) * N(-d1)) + (1.0 / q1) * exp((b - r) * time) * (1.0 / (sigma * time_sqrt)) * n(-d1);
        Si = Si - (g / gprime);
    };
    ...
    if (S > S_star_star) {
        double d1 = (log(S_star_star / X) + (b + 0.5 * sigma_sqr) * time) / (sigma * time_sqrt);
        double A1 = -(S_star_star / q1) * (1 - exp((b - r) * time) * N(-d1));
        P = p + A1 * pow((S / S_star_star), q1);
    } else {
        P = X - S;
    };
    return std::max(P, p);// should not be lower than Black Scholes value
};
